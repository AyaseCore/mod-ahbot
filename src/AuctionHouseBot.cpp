...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        if (!pItem)
        {
			if (debug_Out) TC_LOG_ERROR("misc", "AHBuyer: Item %u doesn't exist, perhaps bought already?", auction->item_guidlow);
            continue;
        }
        ...
        if (debug_Out)
        {
			...
			TC_LOG_INFO("misc", "AHBuyer: Item GUID: %u", auction->item_guidlow);
            TC_LOG_INFO("misc", "AHBuyer: Item Template: %u", auction->item_template);
...
void AuctionHouseBot::IncrementItemCounts(AuctionEntry* ah)
{
    ...
    if (!pItem)
    {
		if (debug_Out) TC_LOG_ERROR("misc", "AHBot: Item %u doesn't exist, perhaps bought already?", ah->item_guidlow);
        return;
    }
...
{
    if (debug_Out)
		TC_LOG_ERROR("misc", "Start Settings for %s Auctionhouses:", WorldDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
    if (AHBSeller)
    {
        ...
        if (debug_Out)
        {
			TC_LOG_ERROR("misc", "Current Settings for %s Auctionhouses:", WorldDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
	...
	if (debug_Out) TC_LOG_ERROR("misc", "End Settings for %s Auctionhouses:", WorldDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
}
